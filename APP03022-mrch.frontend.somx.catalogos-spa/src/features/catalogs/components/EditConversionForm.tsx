...
import { conversionService, catalogService, catalogElementService } from '../../../services/api';
...
export default function EditConversionForm() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      alert(`La conversión ${conversionId} se ha actualizado exitosamente.`);
      navigate(`/catalogos/elementos/${elementId}/conversiones`);
    } catch (error: any) {
  ...
  return (
    <div style={S.container}>
      <div style={{ fontSize: '0.875rem', color: '#64748b', marginBottom: '1.5rem' }}>
        ...
        {' / '}
        <span style={{ color: '#0066CC', cursor: 'pointer' }} onClick={() => navigate('/catalogos/catalogs')}>Gestión de Catálogos</span>
        {' / '}
        <span style={{ color: '#0066CC', cursor: 'pointer' }} onClick={() => navigate('/catalogos/catalogs')}>Catálogos</span>
        {' / '}
        <span style={{ color: '#0066CC', cursor: 'pointer' }} onClick={() => navigate(`/catalogos/elementos/${elementId}/conversiones`)}>Conversiones</span>
        {' / '}
        ...
        <div style={S.modal}>
          <div style={S.modalBox}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.75rem' }}>
              ...
              <button style={{ ...S.btn, ...S.primaryBtn }} onClick={() => navigate(`/catalogos/elementos/${elementId}/conversiones`)}>Sí</button>
            </div>
          </div>
        </div>
      )}
