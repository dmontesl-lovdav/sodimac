...
<xsl:stylesheet version="1.1"
	...
	xmlns:fox="http://xmlgraphics.apache.org/fop/extensions" exclude-result-prefixes="fo util xalan cfdi tfd opamfact fox">
		...
		<fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format">
			...
			<fo:page-sequence master-reference="my-page">
							...
							<fo:table-body>
								<fo:table-row>
									<fo:table-cell display-align="before">
										<fo:block>
											<fo:external-graphic
												src="url({$xsltfilePath}no_logo.jpg)"
												content-width="55mm"
												content-height="40mm"
												scaling="uniform" />
										</fo:block>
									...
									<fo:table-cell display-align="before" text-align="left">
											...
											<fo:table-body>
												<fo:table-row height="9px">
													<fo:table-cell number-columns-spanned="2">
														<fo:block font-size="9pt" font-weight="bold" space-before="1mm" space-after="1mm">
															Emisor
														</fo:block>
													</fo:table-cell>
												</fo:table-row>
										...
										<fo:table>
											...
											<fo:table-body>
												<fo:table-row height="9px">
													<fo:table-cell number-columns-spanned="2">
														<fo:block font-size="9pt" font-weight="bold" space-before="2mm" space-after="2mm">
															Receptor
														</fo:block>
													</fo:table-cell>
												</fo:table-row>
                                ...
                                <fo:table-row height="12px">
                                    <fo:table-cell display-align="center" number-columns-spanned="2" >
                                        ...
                                        <fo:block font-size="6pt" text-align="center" space-before=".5mm" space-after=".5mm" >
                                            <fo:basic-link external-destination="url('https://www.sodimac.com.mx/')"
                ...
                <fo:flow flow-name="Content">
					<fo:block-container absolute-position="fixed" top="16cm" left="3cm" width="21cm" height="6cm" fox:transform="rotate(-45)">
						<fo:block text-align="center" font-size="32pt" font-weight="bold" color="#bbbbbb" line-height="36pt">
                    ...
                    <fo:block space-before="2mm">
	                    ...
	                    <fo:table>
	                    	<fo:table-column column-width="12mm" /><!-- Cve.Prod. Servicio -->
                        	<fo:table-column column-width="23mm" /><!-- No. Identificación -->
                        	<fo:table-column column-width="10mm" /><!-- Cantidad -->
                        	<fo:table-column column-width="10mm" /><!-- Clave Unidad -->
                        	<fo:table-column column-width="10mm" /><!-- Unidad -->
                        	<fo:table-column column-width="24mm" /><!-- Descripción -->
                        	<fo:table-column column-width="12mm" /><!-- Valor Unitario -->
                        	<fo:table-column column-width="14mm" /><!-- Importe -->
                        	<fo:table-column column-width="10mm" /><!-- Descuento -->
                        	<fo:table-column column-width="2mm"  /><!-- Espacio -->
							<fo:table-column column-width="8mm"  /><!-- Objeto Impuesto -->
                        	<fo:table-column column-width="11mm" /><!-- Base -->
                        	<fo:table-column column-width="2mm"  /><!-- Espacio -->
                        	<fo:table-column column-width="10mm"  /><!-- Impuesto -->
                        	<fo:table-column column-width="8mm"  /><!-- Tipo Factor -->
                        	<fo:table-column column-width="8mm"  /><!-- Tasa o C.  -->
                        	<fo:table-column column-width="15mm" /><!-- Importe --> 
                        	...
                        	<fo:table-header>
								...
								<fo:table-row>
                               		<fo:table-cell height="1px" font-weight="bold" number-columns-spanned="17">
                            ...
                            <fo:table-body>
								...
								<xsl:for-each select="./cfdi:Conceptos/cfdi:Concepto">
									...
									<xsl:variable name="v_NoIdentificacion" select="@NoIdentificacion" />
									<xsl:variable name="v_Cantidad" select="@Cantidad" />
									<xsl:variable name="v_ClaveUnidad" select="@ClaveUnidad" />
									<xsl:variable name="v_Unidad" select="@Unidad" />
									...
									<fo:table-row>
	                                 	<fo:table-cell height="2px" font-weight="bold" number-columns-spanned="17">
	                                 		<fo:block></fo:block>
	                                 	</fo:table-cell>
	                                </fo:table-row>
									
									<xsl:for-each select="./cfdi:Impuestos/cfdi:Traslados/cfdi:Traslado">
										<xsl:variable name="v_TipoFactor" select="@TipoFactor" />
										...
										<fo:table-row>
											...
											<fo:table-cell>
								          	...
								          	<fo:table-cell>
								            	<fo:block font-size="5pt" space-before="1mm" space-after="1mm" text-align="center">
	                                                    <xsl:value-of select="$v_NoIdentificacion" />
	                                            </fo:block>
											</fo:table-cell>
								          	<fo:table-cell>
								            	<fo:block font-size="5pt" space-before="1mm" space-after="1mm" text-align="center">
											...
											<fo:table-cell>
								            	<fo:block font-size="5pt" space-before="1mm" space-after="1mm" text-align="center">
								                	<xsl:choose>
								                		<xsl:when test="not(number($v_Unidad))">
					            							<xsl:value-of select="$v_Unidad"/>
					        							</xsl:when>
												        <xsl:when test="contains($v_Unidad, '.')">
												            <xsl:value-of select="concat ( substring-before( $v_Unidad, '.' ) , '.' , substring ( substring-after ( $v_Unidad, '.' ) , 1 , 2 ) )"/>
												        </xsl:when>
												        <xsl:otherwise>
												            <xsl:value-of select="format-number($v_Unidad, '#.##')"/>
												        </xsl:otherwise>
								    				</xsl:choose>
	                                            </fo:block>
											</fo:table-cell>
...
<!-- 	                                               	$<xsl:value-of select="format-number($v_Descuento, '#,##0.00')" /> -->
	                                           	    <xsl:choose>
												        ...
												        <xsl:when test="contains($v_Descuento, '.')">
												            ...
												            $<xsl:value-of select="format-number($v_DescuentoFormat, '#,###.00')"/>
												        </xsl:when>
												        <xsl:otherwise>
												            $<xsl:value-of select="format-number($v_Descuento, '#,###.00')"/>
												        </xsl:otherwise>
								    				 </xsl:choose>
	                                           	</fo:block>
							             	</fo:table-cell>
											...
											<fo:table-cell display-align="after" text-align="center">
												<fo:block font-size="5pt" space-before="1mm" space-after="1mm">
													<xsl:value-of select="$v_ObjetoImp" />
												</fo:block>
									        </fo:table-cell>
									        
											<xsl:if test="$v_TipoFactor != 'tipoFactor'">
									        
									        <fo:table-cell display-align="after" text-align="right">
												<fo:block font-size="5pt" space-before="1mm" space-after="1mm">
<!-- 		                                        	$<xsl:value-of select="format-number(@Base, '#,###.00')" /> -->
													<xsl:choose>
								                		<xsl:when test="not(number(@Base))">
					            							$<xsl:value-of select="@Base"/>
					        							</xsl:when>
												        <xsl:when test="contains(@Base, '.')">
												        	<xsl:variable name="v_trasBase" select="concat ( substring-before( @Base, '.' ) , '.' , substring ( substring-after ( @Base, '.' ) , 1 , 2 ) )" />
												            $<xsl:value-of select="format-number($v_trasBase, '#,###.00')"/>
												        </xsl:when>
												        <xsl:otherwise>
												            $<xsl:value-of select="format-number(@Base, '#,###.00')"/>
												        </xsl:otherwise>
									        ...
									        <fo:table-cell display-align="after" text-align="center">
									        	<fo:block font-size="5pt"  space-before="1mm" space-after="1mm">
<!-- 		                                        	<xsl:value-of select="format-number(@TasaOCuota, '#.##')"/> -->
		                                       	    <xsl:choose>
												        <xsl:when test="contains(@TasaOCuota, '.')">
												            <xsl:value-of select="concat ( substring-before( @TasaOCuota, '.' ) , '.' , substring ( substring-after ( @TasaOCuota, '.' ) , 1 , 2 ) )"/>
												        </xsl:when>
												        <xsl:otherwise>
												            <xsl:value-of select="format-number(@TasaOCuota, '#.##')"/>
												        </xsl:otherwise>
...
<!-- 		                                            	$<xsl:value-of select="format-number(@Importe, '#,###.00')" /> -->
												        ...
												        <xsl:when test="contains(@Importe, '.')">
												        	<xsl:variable name="v_ImporteTransFormat" select="concat ( substring-before( @Importe, '.' ) , '.' , substring ( substring-after ( @Importe, '.' ) , 1 , 2 ) )" />
												            $<xsl:value-of select="format-number($v_ImporteTransFormat, '#,###.00')"/>
												        </xsl:when>
												        <xsl:otherwise>
												            $<xsl:value-of select="format-number(@Importe, '#,###.00')"/>
												        </xsl:otherwise>
									...
									</xsl:for-each><!-- ./cfdi:Impuestos/cfdi:Traslados/cfdi:Traslado -->
								</xsl:for-each><!-- ./cfdi:Conceptos/cfdi:Concepto -->
                                
                                <fo:table-row>
                                 	<fo:table-cell height="2px" number-columns-spanned="17">
                                 		<fo:block></fo:block>
                                 	</fo:table-cell>  
                                </fo:table-row>
                                ...
                                <fo:table-row>
									<fo:table-cell height="2px" number-columns-spanned="14">
										<fo:block font-size="5pt" space-before="1mm" space-after="1mm">
											<fo:table>
