...
export const CreateAuditLogSchema = z
    .object({
        idTransaccion: UUID,
        idAplicativo: z.string().min(1), 
        idModulo: z.string().min(1),     
        paso: z.string().min(1),
        detalle: z.string().min(1).optional().nullable(), 
        fechaHora: z.coerce.date().optional(), 
        tipoEvento: AuditTipoEventoSchema,
        idUsuario: z.string().min(1).optional().nullable(), 

        idError: z.string().min(1).optional().nullable(),   
        idMensaje: z.string().min(1).optional().nullable(),
        mensaje: z.string().min(1).optional().nullable(),   
        log: z.string().min(1).optional().nullable(),       
    })
    .superRefine((v, ctx) => {
