...
public class BatchDatabaseMigration {
    ...
    private void verifyMigration(JdbcTemplate jdbcTemplate) {
        try {
            ...
            String countSpQuery = "SELECT COUNT(*) FROM sys.procedures";
            Integer spCount = jdbcTemplate.queryForObject(countSpQuery, Integer.class);
            log.info("✓ Stored Procedures creados: {}", spCount);

            // Verificar vistas
            String countViewsQuery = "SELECT COUNT(*) FROM INFORMATION_SCHEMA.VIEWS";
            Integer viewCount = jdbcTemplate.queryForObject(countViewsQuery, Integer.class);
            log.info("✓ Vistas creadas: {}", viewCount);
