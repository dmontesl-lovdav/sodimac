...
if (authPublicKey) {
  try {
    decodedAuthKey = new X509Certificate(
      Buffer.from(authPublicKey, "base64")
    ).toString();

    logger.info(" AUTH CERT LOADED SUCCESSFULLY ");
  } catch (error) {
    logger.warn(
      " AUTH CERT NOT CONFIGURED - JWT validation disabled "
    );
  }
} else {
  logger.warn(
...
const remoteResolver = proxy(remoteUrl, {
  ...
  proxyReqPathResolver: (request) => {
    const targetPath = request.originalUrl.replace(
      localContext,
      ""
    );
  ...
  proxyErrorHandler: (error, response, nextFilter) => {
    if (error?.code) {
      logger.warn(
        `ABORTING REQUEST WITH CODE ${error.code}`
      );
    } else {
      logger.warn(
        `ABORTING REQUEST WITH UNKNOWN CODE: ${error}`
      );
    }
...
localService
  ...
  .on("request", (request, response) => {
    logger.info(
      `ACCEPTING NEW REQUEST ${request.method}: ` +
      `${request.originalUrl} - ` +
