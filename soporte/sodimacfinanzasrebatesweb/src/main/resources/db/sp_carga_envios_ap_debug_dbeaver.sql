-- ====================================================================
-- STORED PROCEDURE DE DEBUG PARA sp_carga_envios_ap
-- ====================================================================
-- VERSION COMPATIBLE CON DBEAVER
-- ====================================================================
-- Ejecutar con: EXEC sp_carga_envios_ap_debug @V_NOMBRE_ARCHIVO, @V_IDUSUARIO, @DEBUG_MODE
-- Ejemplo:
--   EXEC sp_carga_envios_ap_debug
--        'Conversion_Envios_Ap_v2 varios fill rate Agosto2025.5.csv',
--        2,
--        1  -- 1 = No ejecutar TRUNCATE, 0 = Ejecutar normal
-- ====================================================================

-- Primero eliminar el SP si existe
IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'sp_carga_envios_ap_debug')
    DROP PROCEDURE [dbo].[sp_carga_envios_ap_debug];

-- Crear el SP
CREATE PROCEDURE [dbo].[sp_carga_envios_ap_debug]
    @V_NOMBRE_ARCHIVO VARCHAR(100),
    @V_IDUSUARIO NUMERIC,
    @DEBUG_MODE BIT = 1  -- 1 = No ejecutar TRUNCATE, 0 = Ejecutar normal
AS
BEGIN
    SET NOCOUNT ON;
    ...
    PRINT '========================================';
    PRINT 'Archivo: ' + ISNULL(@V_NOMBRE_ARCHIVO, 'NULL');
    PRINT 'Usuario: ' + CAST(@V_IDUSUARIO AS VARCHAR);
    PRINT 'Modo DEBUG: ' + CASE WHEN @DEBUG_MODE = 1 THEN 'ACTIVO (No se borraran datos)' ELSE 'INACTIVO' END;
    PRINT '';
    ...
    IF @V_NUM_REGISTRO = 0
    BEGIN
        PRINT 'ERROR: No hay registros para procesar (Codigo 3)';
    ...
    IF @V_CONTADOR > 0
    BEGIN
        PRINT 'ERROR: Existen documentos duplicados (Codigo 2)';
    ...
    PRINT 'Registros con tipo de documento invalido: ' + CAST(ISNULL(@V_CONTADOR, 0) AS VARCHAR);

    IF @V_CONTADOR > 0
    BEGIN
        PRINT 'ERROR: Tipo de documento invalido (Codigo 5)';
        PRINT 'Tipo esperado: ' + ISNULL(@V_TIPO_DOCUMENTO, 'NULL');
    ...
    ELSE
    BEGIN
        PRINT 'OK: Tipo de documento valido';
    END
    PRINT '';
    ...
    IF @V_CONTADOR > 0
    BEGIN
        PRINT 'ERROR: Tipo de cambio invalido para moneda extranjera (Codigo 6)';
    ...
    ELSE
    BEGIN
        PRINT 'OK: Tipo de cambio valido para moneda extranjera';
    END
    PRINT '';
    ...
    IF @V_CONTADOR > 0
    BEGIN
        PRINT 'ERROR: Tipo de cambio debe ser 1 para MXN (Codigo 7)';
    ...
    ELSE
    BEGIN
        PRINT 'OK: Tipo de cambio valido para MXN';
    END
    PRINT '';
    ...
    BEGIN TRY
        ...
        IF @V_CONTADOR > 0
        BEGIN
            PRINT 'ERROR: Suma de sucursales invalida (Codigo 8)';
        ...
        ELSE
        BEGIN
            PRINT 'OK: Suma de sucursales valida';
        END

    END TRY
    BEGIN CATCH
        PRINT 'ERROR: No se puede convertir SUCURSAL a numerico (Codigo 9)';
        PRINT 'Error SQL: ' + ISNULL(ERROR_MESSAGE(), 'NULL');

        -- Buscar que registro tiene el problema
        SELECT TOP 5
            ID,
            SUCURSAL,
            NUMERO_DOCUMENTO,
    ...
    BEGIN TRY
        ...
        PRINT 'Registros que se insertarian en Envios_Ap_Manual: ' + CAST(@RegistrosAInsertar_Manual AS VARCHAR);
        ...
        -- Verificar conversion de fechas
        PRINT 'Verificando conversion de fechas...';
        SELECT TOP 1
            FECHA_DOCUMENTO,
        ...
        PRINT 'OK: Conversion de fechas exitosa';

    END TRY
    BEGIN CATCH
        PRINT 'ERROR en INSERT a Envios_Ap_Manual';
        PRINT 'Error SQL: ' + ISNULL(ERROR_MESSAGE(), 'NULL');
        PRINT 'Error Number: ' + CAST(ISNULL(ERROR_NUMBER(), 0) AS VARCHAR);
        PRINT 'Error Line: ' + CAST(ISNULL(ERROR_LINE(), 0) AS VARCHAR);
        GOTO FIN_SP;
    END CATCH
    PRINT '';
    ...
    BEGIN TRY
        ...
        PRINT 'Registros que se insertarian en SODIMAC_SAP_PROD: ' + CAST(@RegistrosAInsertar_SAP AS VARCHAR);
    ...
    BEGIN CATCH
        ...
        PRINT 'Error Line: ' + CAST(ISNULL(ERROR_LINE(), 0) AS VARCHAR);
        PRINT '';
        PRINT 'POSIBLE CAUSA: Problema de conexion a BD externa o estructura de tabla diferente';
        GOTO FIN_SP;
    END CATCH
    PRINT '';
    ...
    END TRY
    BEGIN CATCH
        PRINT 'ERROR en preparacion de datos para controlDocumento';
        PRINT 'Error SQL: ' + ISNULL(ERROR_MESSAGE(), 'NULL');
    END CATCH
    PRINT '';
    ...
    PRINT '========================================';
    PRINT 'Codigo resultado: ' + CAST(@v_RESULTADO AS VARCHAR);
    PRINT 'Descripcion: ' + ISNULL(@V_DESC, 'Sin errores detectados hasta este punto');
    PRINT '';

    -- Retornar el codigo y descripcion como resultado
    SELECT
        @v_RESULTADO AS CodigoResultado,
        ISNULL(@V_DESC, 'Sin errores detectados hasta este punto') AS Descripcion;
END;
