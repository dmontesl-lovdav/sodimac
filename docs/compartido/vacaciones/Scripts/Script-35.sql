...
BEGIN
    ...
    RAISE NOTICE '    - Insertando con payment_uuid: %', v_payment_uuid;
    RAISE NOTICE '    - Relacionado con payments_uuid: %', v_payments_uuid;
    ...
    BEGIN
        ...
        IF NOT EXISTS (SELECT 1 FROM tenant_fiscal.payment WHERE payment_uuid = v_payment_uuid) THEN
            RAISE EXCEPTION 'ERROR: Payment no se insertó correctamente';
        END IF;
        RAISE NOTICE '    ✓ Verificación: Payment existe en la base de datos';

    EXCEPTION
        WHEN OTHERS THEN
            RAISE NOTICE '    ✗ ERROR insertando payment: %', SQLERRM;
            RAISE;
    END;
    ...
    -- 4. Crear RELATED_DOCUMENTS (vincular pago con invoice) usando INSERT SELECT
    ...
    SELECT
        ...
        v_payment_amount,
        i.total,
        i.total - v_payment_amount,  -- Saldo restante
    ...
    CROSS JOIN tenant_fiscal.invoice i
    WHERE p.payment_uuid = v_payment_uuid
      AND i.invoice_uuid = v_invoice_uuid;
