...
export default function ShippingGuideStatusUpdate(): ReactElement {
  ...
  useEffect(() => {
    const fetchDetail = async (id: string) => {
      setLoading(true);
      try {
        const d = await client.getDetail(id);
        setDetail(d);
    ...
    const fetchHistory = async (id: string) => {
      try {
        const h = await client.getStatusHistory(id);
        setHistory(h);
      } catch {
        setHistory([]);
      }
  ...
  useEffect(() => {
    // si el target actual no está permitido, reset
    if (
    ...
    if (targetStatus === 3) {
      if (!series || !folio || !uuid || !comment.trim()) {
        setAlertModal({
          ...
          message: "Serie, Folio, UUID y Comentario son obligatorios para Consumida manual.",
          severity: "warning",
        });
        return false;
      }
      if (series.length > 100 || folio.length > 100 || uuid.length > 36 || comment.length > 254) {
        setAlertModal({
          visible: true,
          title: "Longitudes excedidas",
          message: "Revisa la longitud: Serie/Folio (100), UUID (36), Comentario (254).",
          severity: "warning",
        });
        return false;
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setAlertModal({
        ...
        message: "La guía de embarque ha cambiado de estatus con éxito.",
        severity: "success",
      });
  ...
  const renderHistory = () => {
    if (!history || history.length === 0) {
      return <div className="sg-divider">Sin histórico disponible</div>;
    }
    return (
      <div className="sg-divider">
        ...
        <table className="gt-table" style={{ marginTop: 8 }}>
          <thead className="gt-thead">
            <tr>
              <th className="gt-th gt-align-left">Fecha registro</th>
              <th className="gt-th gt-align-left">Estatus</th>
              <th className="gt-th gt-align-left">Usuario</th>
              <th className="gt-th gt-align-left">Comentario</th>
            </tr>
          </thead>
          <tbody>
            {history.map((h, idx) => (
              <tr key={idx} className="gt-row">
                <td className="gt-td">{new Date(h.registeredAt).toLocaleString("es-MX")}</td>
                <td className="gt-td">
                  {statusOptions.find((s) => s.value === h.status)?.label || h.status}
                </td>
                <td className="gt-td">{h.userId || "N/D"}</td>
                <td className="gt-td">{h.comment || "N/D"}</td>
              </tr>
            ))}
          </tbody>
        </table>
  ...
  return decorate(
    ...
    <div className="space-y-4">
      <div className="sg-header">
        <div className="sg-title">Actualizar estatus</div>
      </div>
      <div className="sg-divider" />
      ...
      <div className="grid gap-4" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))" }}>
        <div>
          <label className="sg-title" style={{ fontSize: "12px", fontWeight: 600 }}>
            Estatus actual
          </label>
          <div>{statusOptions.find((s) => s.value === currentStatus)?.label || "N/D"}</div>
        </div>
        <div>
          <label className="sg-title" style={{ fontSize: "12px", fontWeight: 600 }}>
            Nuevo estatus
          ...
          <select
            value={targetStatus ?? ""}
            onChange={(e: ChangeEvent<HTMLSelectElement>) => handleStatusChange(Number(e.target.value))}
            className="sg-select"
          >
            <option value="">Selecciona estatus</option>
            {statusOptions
              .filter((s) => allowedTargetsForCurrent.includes(s.value))
              .map((s) => (
        ...
        <div>
          ...
          <select
            value={reasonId}
            onChange={(e: ChangeEvent<HTMLSelectElement>) => setReasonId(e.target.value)}
            className="sg-select"
          >
      ...
      <div className="sg-modal-footer">
        <button className="sg-btn sg-btn-outline" onClick={() => nav(-1)}>
          Regresar
        </button>
        <button className="sg-btn sg-btn-primary" onClick={handleSubmit} disabled={loading}>
