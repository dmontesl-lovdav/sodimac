...
export async function attachSecurityContext(req: Request, res: Response, next: NextFunction): Promise<void> {
    if (!isSecurityEnabled()) {
        // Dev / security disabled: sin restriccion (acceso total)
    ...
    if (!ctx) {
        // util-api inalcanzable o usuario no encontrado → sin restriccion permisiva
        // (alternativa: 503 estricto. Aqui preferimos no romper requests).
        req.security = { vendors: null, types: null, groups: null };
        return next();
    }
...
// Test helper: limpiar cache (no usar en prod)
