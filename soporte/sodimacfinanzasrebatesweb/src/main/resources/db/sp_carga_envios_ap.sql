...
*/
CREATE PROCEDURE [dbo].[sp_carga_envios_ap] 
...
BEGIN TRY
    BEGIN TRANSACTION
		...
		LEFT(LTRIM(RTRIM(MONEDA)), 3) MONEDA,	
		TIPO_CAMBIO,
		LEFT(DEBITO_CREDITO, 1) DEBITO_CREDITO,
		LEFT(CUENTA_CONTABLE, 12) CUENTA_CONTABLE,
		LEFT(CODIGO_PROVEEDOR, 10) CODIGO_PROVEEDOR,
		IMPORTE,
		LEFT(SUCURSAL, 5) SUCURSAL,
		LEFT(CONDICION_PAGO, 4) CONDICION_PAGO,
		...
		LEFT(CLASE_DOCUMENTO, 2) CLASE_DOCUMENTO,
		LEFT(NUMERO_REFERENCIA, 18) NUMERO_REFERENCIA,
		LEFT(CENTRO_COSTO, 10) CENTRO_COSTO,
		LEFT(CENTRO_BENEFICIO, 10) CENTRO_BENEFICIO,
		(CASE WHEN LTRIM(RTRIM(NUMERO_UUID)) = '' THEN NULL ELSE LEFT(LTRIM(RTRIM(NUMERO_UUID)), 36) END) NUMERO_UUID,
		FLAG_ENVIADO,
		CONVERT(DATE,FECHA_RECEPCION,103) FECHA_RECEPCION,
		LEFT(LTRIM(RTRIM(TIPO_DOCUMENTO)), 20) TIPO_DOCUMENTO,
		'SODISAP' ORIGEN_ETL,	
		IdPeriodo,
		Timbrado,
		TipoRebate
