...
import com.sodimac.rebates.service.IRebateOrdenCompraService;
import com.sodimac.rebates.service.ITiendaService;
import com.sodimac.rebates.service.ITipoOrdenCompraService;
import com.sodimac.rebates.util.ExportOrdenCompraExcel;

@Controller
@RequestMapping("/ordenCompra")
public class RebateOrdenCompraController extends BaseController {
	...
	@Autowired
	private IRebateOrdenCompraService ordenCompraService;
	...
	public String index(Model model) {
		...
		List<CatEstadoOrdenCompraEntity> estadosOrdenCompra = this.estadoOrdenCompraService.getEstadosOrdenCompra().stream().filter(x -> x.getIdEstadoOrdenCompra() > 2).collect(Collectors.toList());
	...
	@PostMapping("/consult")
	public String consult(@RequestBody ReporteOrdenCompra ordenCompra
			 			, BindingResult result
			 			, Model model) {
		Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
		Sesion sesion = (Sesion) authentication.getPrincipal();
		List<RebateOrdenCompraEntity> ordenesCompra = null;
		if (result.hasErrors()) {
			...
			return "fragments/ordenCompra :: start";
		}

		if (ordenCompra.getFechaFin().before(ordenCompra.getFechaIni())) {
        ...
        inicio.setTime(ordenCompra.getFechaIni());
        fin.setTime(ordenCompra.getFechaFin());
		...
		ordenesCompra = this.ordenCompraService.getOrdenCompra(ordenCompra);
		if (ordenesCompra.size() >= 1) {
			sesion.setReporteOrdenCompra(ordenCompra);
			model.addAttribute("lista", ordenesCompra);
			model.addAttribute("ordenCompraFilter", ordenCompra);
			return "fragments/ordenCompra :: table";
		}
		return "fragments/ordenCompra :: noRecord";
	}
	...
	public void exportReportUser(HttpServletResponse response)
		...
		String name_report_user = nombreMes + anio.toString() + " OrdenCompra " + strDateComplete + ".xlsx";
		response.setContentType("application/octet-stream");
		response.setHeader("Content-Disposition", "attachment; filename=" + name_report_user);

		ReporteOrdenCompra ordenCompra = sesion.getReporteOrdenCompra();
		ordenCompra.setRowsPerPage(-1);
		List<RebateOrdenCompraEntity> dataReport = this.ordenCompraService.getOrdenCompra(ordenCompra);
		ByteArrayInputStream stream = ExportOrdenCompraExcel.ordenCompraListToExcelFile(dataReport);
