...
-- ============================================================
-- TABLAS PERSONALIZADAS PARA INVOICE STATUS SYNC
-- ============================================================

-- Tabla: CtrlEnlace (Log de ejecuciones del batch)
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'CtrlEnlace')
BEGIN
    CREATE TABLE CtrlEnlace
    (
        ...
        CONSTRAINT PK_CtrlEnlace PRIMARY KEY CLUSTERED (id)
    );

    CREATE INDEX IX_CtrlEnlace_execution_id ON CtrlEnlace(execution_id);
    CREATE INDEX IX_CtrlEnlace_start_time ON CtrlEnlace(start_time DESC);
END

-- Tabla: CtrlEnlace_CifrasControl (Fotografía de cifras antes/después)
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'CtrlEnlace_CifrasControl')
BEGIN
    CREATE TABLE CtrlEnlace_CifrasControl
    (
        id INT IDENTITY(1,1) NOT NULL,
        ...
        status_breakdown NVARCHAR(500),
        CONSTRAINT PK_CtrlEnlace_CifrasControl PRIMARY KEY CLUSTERED (id)
    );

    CREATE INDEX IX_CtrlEnlace_CifrasControl_execution_id ON CtrlEnlace_CifrasControl(execution_id);
    CREATE INDEX IX_CtrlEnlace_CifrasControl_phase ON CtrlEnlace_CifrasControl(phase);
END

-- Tabla: CtrlEnlace_Comprobante_ControlDocumento (Detalle de cada transición)
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'CtrlEnlace_Comprobante_ControlDocumento')
BEGIN
    CREATE TABLE CtrlEnlace_Comprobante_ControlDocumento
    (
        id INT IDENTITY(1,1) NOT NULL,
        execution_id VARCHAR(36) NOT NULL,
        ...
        CONSTRAINT PK_CtrlEnlace_Comprobante PRIMARY KEY CLUSTERED (id)
    );

    CREATE INDEX IX_CtrlEnlace_Comprobante_execution_id ON CtrlEnlace_Comprobante_ControlDocumento(execution_id);
    CREATE INDEX IX_CtrlEnlace_Comprobante_uuid ON CtrlEnlace_Comprobante_ControlDocumento(uuid);
    CREATE INDEX IX_CtrlEnlace_Comprobante_success ON CtrlEnlace_Comprobante_ControlDocumento(success);
END

-- Tabla: CtrlEnlaceLog (Registro de errores)
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'CtrlEnlaceLog')
BEGIN
    CREATE TABLE CtrlEnlaceLog
    (
        id INT IDENTITY(1,1) NOT NULL,
        execution_id VARCHAR(36) NOT NULL,
        ...
        CONSTRAINT PK_CtrlEnlaceLog PRIMARY KEY CLUSTERED (id)
    );

    CREATE INDEX IX_CtrlEnlaceLog_execution_id ON CtrlEnlaceLog(execution_id);
    CREATE INDEX IX_CtrlEnlaceLog_logged_at ON CtrlEnlaceLog(logged_at DESC);
END

-- Tabla: AdminCatalogo (Ya existe arriba, esta es solo documentación)
-- Nota: AdminCatalogo ya fue creada en la línea 22-42
