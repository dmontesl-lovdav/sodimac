...
@Component
public class PortalInHouseToEstFacturasJob implements SodimacJob {

	private static final Logger logger = LoggerFactory.getLogger(PortalInHouseToEstFacturasJob.class);
	...
	@Autowired
	private EstFacturasService estFacturasService;
	...
	public void sincroniza() {
		...
		boolean rangoFechas = Boolean.valueOf(env.getProperty("factutas.to.bct.rango.fechas")).booleanValue();
		int diasAtraso = Integer.valueOf( env.getProperty("factutas.to.bct.dias") ).intValue();
		String fechaInicio = env.getProperty("factutas.to.bct.fecha.inicio");
		String fechaFin = env.getProperty("factutas.to.bct.fecha.fin");
	...
	private void sincronizar(String pFechaInicio, String pFechaFin) throws ParseException {
		...
		while (calInicio.getTime().compareTo(calFin.getTime()) <= 0) {
			String fechaSincroniza = sdf.format(calInicio.getTime());
			 logger.info("Inicia sincronización de Venta MSI");
			 logger.info("FECHA: " + fechaSincroniza);
			 
			 this.sincronizar(fechaSincroniza);
			 
			 calInicio.add(Calendar.DATE, 1);
		     logger.info("Finaliza sincronización de Venta MSI");
		}
	}
	...
	private void sincronizar(String pFecha) throws ParseException {
		...
		if (listTicket != null && listTicket.size() > 0) {
			logger.info("Total de ticket a revisar: " + listTicket.size());
			
			List<List<FacturasEntity>> batches = this.getBatches(listTicket, 800);
			if (batches != null && batches.size() > 0) {
				
				int chunk = 0;
				for (List<FacturasEntity> entities : batches) {
		
					logger.info("chunk: " + chunk);
					chunk ++;
					
					List<String> uuids = this.getUuids( entities );
					List<String> tickets = this.estFacturasService.getTickets(uuids);
					List<FacturasEntity> listTicketFinal = this.getFacturasFinales(entities, tickets);
					if (listTicketFinal != null) {
						logger.info("Total de ticket a sincronizar: " + listTicketFinal.size());
						int i=1;
						for (FacturasEntity facEnt : listTicketFinal) {
							logger.info("TicketBct: " + facEnt.getTicketBct() + " - " + facEnt.getUuid() + " - [" + i + " de " + listTicketFinal.size() + "]");
							
							Integer existeTicket = this.estFacturasService.existeTicket(facEnt.getUuid());
							...
							if (existeTicket.intValue() == 0 ) {
								...
								this.estFacturasService.insertar(facturaModel);
	...
	private List<FacturasEntity> getFacturasFinales(List<FacturasEntity> listTicket, List<String> uuids) {
		List<FacturasEntity> listTicketFinal = new ArrayList<FacturasEntity>(0);
	
		if (listTicket != null && listTicket.size() > 0) {
			nextUuid:
			for (FacturasEntity facEnt : listTicket) {
				for (String uuid : uuids) {
					if (facEnt.getUuid().equals( uuid )) {
						logger.info("TicketBct: " + facEnt.getTicketBct() + " - " + facEnt.getUuid() + " - [EXISTE]");
						continue nextUuid;
					}
				}
				
				logger.info("TicketBct: " + facEnt.getTicketBct() + " - " + facEnt.getUuid() + " - [NO EXISTE]");
				listTicketFinal.add(facEnt);
			}
		}
		return listTicketFinal;
	}

	private List<String> getUuids(List<FacturasEntity> listTicket) {
		List<String> list = new ArrayList<String>();
		for (FacturasEntity entity : listTicket) {
