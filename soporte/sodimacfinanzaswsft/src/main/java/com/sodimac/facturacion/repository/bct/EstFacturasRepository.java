...
public class EstFacturasRepository {
	...
	public String findUuidByTicket(String ticket) {
		...
		String sql = "select NUM_FACTURA from EST_FACTURAS where NUM_TRX = '{ticket}' and NUM_TIENDA is null and FECHA_CIERRE IS NULL and NUM_FACTURA IS not NULL order by NUM_FACTURA";
	...
	public String ValidateInvoiceByTicket(String ticket) {

		String sql = "SELECT X.NUM_TRX FROM EST_FACTURAS X WHERE X.FECHA_CIERRE IS NULL AND X.NUM_TIENDA IS NULL and NUM_FACTURA IS not NULL AND NUM_TRX = '{ticket}'";
		sql = sql.replace("{ticket}", ticket);
		@SuppressWarnings("unchecked")
		List<String> list = (List<String>) QueryBctRepository.executeGetResultList(sql);
