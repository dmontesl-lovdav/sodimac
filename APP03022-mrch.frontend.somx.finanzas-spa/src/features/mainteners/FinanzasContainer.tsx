...
export default function FinanzasContainer({
    cards,
}: {
    cards?: FinanzasCard[];
}) {
    ...
    const [modalVariant, setModalVariant] =
        useState<'loading' | 'alert'>('loading');
    ...
    async function handleHealthcheck() {
        try {
            ...
            setModalTitle(
                response.alive
                    ? 'Servicio activo'
                    : 'Servicio sin respuesta válida'
            );

            setModalMessage(
                response.alive
                    ? `Healthcheck exitoso. Servicio: ${firstRecord?.serviceName ??
                    'N/A'
    ...
    const DEFAULT_CARDS: FinanzasCard[] = [
        {
            ...
            description:
                'Visualiza las guías de embarque registradas en el sistema.',
            link: '/finanzas/guias',
            icon: iconSupport,
            requiredApp:
                APP_KEYS.CARTA_PORTE,
        },
        {
            title: 'Lista de Recepciones',
            description:
                'Consulta las recepciones de las órdenes de servicio y su estatus.',
        ...
        },
        {
            ...
            description:
                'Consulta y gestiona los descuentos comerciales de la aplicación.',
            link:
                '/finanzas/descuentos-comerciales',
            icon: iconSetting,
            requiredApp:
                APP_KEYS.DISCOUNTS,
        },
        {
            title: 'Pagos',
            description:
                'Consulta y gestiona los pagos de proveedores.',
            link: '/finanzas/pagos',
            icon: iconDoneCheck,
            requiredApp:
                APP_KEYS.PAYMENTS,
        },
        {
            ...
            description:
                'Consulta los estados de cuenta de proveedores y genera el documento en PDF.',
            link:
                '/finanzas/estado-cuenta',
            icon: iconReport,
            requiredApp:
                APP_KEYS.ACCOUNT_STATEMENT,
        },
        {
            title: 'Three Way Match',
            description:
                'Consulta y validación de orden de compra, recepción y factura pendientes de pago o pagadas.',
            link:
                '/finanzas/three-way-match',
            icon: iconWarning,
            requiredApp:
                APP_KEYS.THREE_WAY_MATCH,
        },
        {
            title:
                'Publicación de recepción MIGO',
            description:
                'Consultar, publicar, autorizar o rechazar recepciones MIGO',
            link: '/finanzas/migo',
            icon: iconDocument,
            requiredApp:
                APP_KEYS.MIGO,
        },
        {
            title: 'Healthcheck',
            description:
                'Valida si el servicio de Finanzas y la conexión a base de datos están activos.',
            icon: iconDoneCheck,
            onClick: () => {
        ...
        <div className="finanzas-root">
            ...
            <main className="finanzas-main">
                <section className="finanzas-box">
                    <h1 className="maintainers-title">
                    ...
                    <section className="cards-grid">
                        {finalCards.map(
                            (it) => {
                                ...
                                /*
                                 * LOCAL:
                                 * se omite PermissionGate para
                                 * poder visualizar todas las cards
                                 * sin autenticación.
                                 *
                                 * UAT / PROD:
                                 * se conserva la validación normal.
                                 */
                                if (
                                    !isLocal &&
                                    (it.requiredApp ||
