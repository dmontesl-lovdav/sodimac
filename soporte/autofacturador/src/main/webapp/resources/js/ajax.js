$(document).ready(function(){
    ...
    $.ajax({
    	url: url,
    	data: {NombreCampo:"Configuracion.RFC.PublicoGeneral"},
    	type: "post", async: true, cache: false, crossDomain: false,
    	success: function(data){
    		$("#hdnRFCPublicoGeneral").val(data);
    	}
    });
    ...
    $.ajax({
    	url: url,
    	data: {NombreCampo:"Configuracion.RFC.PublicoGeneral.Mensaje"},
    	type: "post", async: true, cache: false, crossDomain: false,
    	success: function(data){
    		$("#hdnRFCPublicoGeneralMensaje").val(data);
    	}
    });
	...
	//$.getJSON('https://json.geoiplookup.io/api?callback=?', function(data) {
	 ...
	 // Opera 8.0+
	   ...
	   var browser = browsers.filter(function (_browserItem){
		...
		var url = '/facturacion/InicializarBitacora';
		$.ajax({
			url: url,
...
function rfcValidation(rfc){
    if (!rfcValidarExpReg(rfc)){
        ...
        var result = false
        var url = '/facturacion/ValidarRfc';
        $.ajax({
        	url: url,
        	data: {rfc: rfc},
...
function rfcValidarExpReg(rfc){
    var re = new RegExp($("#hdnExpresionRegularRfc").val());
    return re.test(rfc);
}

//METODO PARA INGRESAR SUS SERVICIOS para verificar si existe el RFC en DBs.
function existeRFC(rfc){
	var result;
    var url = '/facturacion/ObtenerCliente';
    $.ajax({
    	url: url,
    	data: {rfc: rfc},
...
//METODO PARA INGRESAR SUS SERVICIOS para verificar si existe el RFC en DBs. valida la versión del CFDI
function existeRFCVersion(rfc, versionCfdi){
	var result;
    var url = '/facturacion/ObtenerClienteVersion';
    $.ajax({
    	url: url,
    	data: {rfc: rfc,
...
function validarCP(codigoPostal){
	var result;
    var url = '/facturacion/ValidarCodigoPostal';
    $.ajax({
    	url: url,
...
//METODO PARA obtener versión CFDI
function versionCFDI(idAplicacion){
	var result;
    var url = '/facturacion/VersionTimbrado';
    $.ajax({
    	url: url,
...
function versionEspecificaCFDI(ticket,version, vvEE){
	var result;
    var url = '/facturacion/VersionEspecificaTimbrado';
    $.ajax({
    	url: url,
    	data: {ticket: ticket,
    		   version: version,
...
//METODO PARA validar razon social
function validarRegimenCapital(razonSocial){
	var result;
    var url = '/facturacion/ValidarRegimenCapital';
    $.ajax({
    	url: url,
...
function obtenerClaveUsoCfdiNotaCredito(){
	var result;
    var url = '/facturacion/ObtenerClaveUsoCfdiNotaCredito';
    $.ajax({
    	url: url,
    	type: "post", async: false, cache: false, crossDomain: false,
    	success: function(data){
...
function obtenerDatosCfdiNotaCredito(){
	var result;
    var url = '/facturacion/ObtenerDatosCfdiNotaCredito';
    $.ajax({
    	url: url,
    	type: "post", dataType: "json", async: false, cache: false, crossDomain: false,
    	success: function(data){
...
function consultarUsoCfdiRegimenFiscal(rfc, version, regimenFiscal){
	var result;
    var url = '/facturacion/UsoDeCfdiVersion';
    showSpinner();
    $.ajax({
    	url: url,
    	data: {rfc: rfc,
...
function visualizarPDF(uuid)
{
    ...
    var url = '/facturacion/VisualizarPdf';
    $.ajax({
    	url: url,
...
function reenviarFactura()
{
	...
	var url = '/facturacion/ReenvioFactura';
...
function eventosReenvio(){
    ...
    releaseEventCopiPaste("#correo_electronico_anterior", "paste", "#correo_electronico_anterior_validation", "El sistema no permite pegar, ¡Gracias!");
    releaseEventCopiPaste("#correo_electronico_anterior", "copy", "#correo_electronico_anterior_validation", "El sistema no permite copiar, ¡Gracias!");
    releaseEventCopiPaste("#correo_electronico_anterior", "dragover", "#correo_electronico_anterior_validation", "El sistema no permite drag & drop, ¡Gracias!");
    releaseEventCopiPaste("#correo_electronico_nuevo", "paste", "#correo_electronico_nuevo_validation", "El sistema no permite pegar, ¡Gracias!");
    releaseEventCopiPaste("#correo_electronico_nuevo", "copy", "#correo_electronico_nuevo_validation", "El sistema no permite copiar, ¡Gracias!");
    releaseEventCopiPaste("#correo_electronico_nuevo", "dragover", "#correo_electronico_nuevo_validation", "El sistema no permite drag & drop, ¡Gracias!");
}
...
function validateRS(razonSocial) {

	var re = new RegExp($("#hdnExpresionRegularRZ").val());
    return re.test(razonSocial);
}

function validateObra(nombreObra) {

	var re = new RegExp($("#hdnExpresionRegularObra").val());
    return re.test(nombreObra);
}

function validateResponsableObra(responsableObra) {

	var re = new RegExp($("#hdnExpresionRegularResponsableObra").val());
    return re.test(responsableObra);
}
...
function verificaTabTicket(event, id, validateId, number){
    var x = event.keyCode;  
    var subId = "#"+id;
    if((x >= 48 && x<= 57) || (x >= 96 && x<= 105) ||  (x >= 65 && x<= 90) || (x==13) ||  (x==9) || (x==32) ||  (x==8) ||  (x==46) || (37 <= x && x <= 40)){
...
function validarticket(e) {
	...
	if ((keyCode == 8) ||  (keyCode == 46) || (37 <= keyCode && keyCode <= 40) || isValid && !e.shiftKey) {
		return isValid;
	} else {
		e.preventDefault();
	}
...
function onkeypressverifica(event) {
	var x = event.keyCode;
	
	//39=' 42=* 43=+ 45=- 47=/ 125=} 161=! 168=dieresis 180=acento 186=circulo 222={ 123=´
	if (x==39 || x==42 || x==45 || x==47 || x==125 || x==43 || x==161 || x==168 || x==180 || x==186 || x==222 || x==123) {
		 event.preventDefault();
	}
}

function verificaTabRFC(event){
    if (event.keyCode==9 || event.keyCode==13) {
        validaRFC();
        return;
    }
    ...
    if (field.length == 0) {
        ...
        if (method == "email"){
            ...
            if (validateEmail(mailValidate)){
                ...
                var cadenaTwo = success.substring(space+1, success.length);
                element.classList.add(cadenaOne, cadenaTwo);
//                var activarMail = document.getElementById("enviarUserDate");
//                activarMail.removeAttribute("disabled");                        
            }else {
...
function btnDescargaMultiple_onClick() {
	...
	var url = '/facturacion/DescargarMultiple/'+archivos;
