...
--   preferred_username -> nombre / alias de login
...
--  - catalog_detail_attribute_value_id -> shared_catalogs.catalog_detail.id (valor del atributo)
...
CREATE TABLE IF NOT EXISTS core_security.user_attribute (
    ...
    catalog_detail_attribute_value_id INTEGER NOT NULL,
    status SMALLINT NOT NULL DEFAULT 1,
    created_by VARCHAR(80) NOT NULL DEFAULT 'SYSTEM',
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_by VARCHAR(80),
    ...
    CONSTRAINT fk_user_attribute_attribute_value
        FOREIGN KEY (catalog_detail_attribute_value_id) REFERENCES shared_catalogs.catalog_detail (id)
);
...
CREATE INDEX IF NOT EXISTS idx_user_attribute_attribute_value
    ON core_security.user_attribute (catalog_detail_attribute_value_id);
