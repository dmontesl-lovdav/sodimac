...
import com.sodimac.rebates.service.ICalculoRebateMSIService;
import com.sodimac.rebates.service.IPeriodoService;
import com.sodimac.rebates.util.ExportCalculoRebateMSIExcel;

@Controller
@RequestMapping("/rebatesMSI")
public class CalculoRebateMSIController extends BaseController {

	private static int DIFERENCIA_MESES = 6;
	
	@Autowired
	private ICalculoRebateMSIService iCalculoRebateMSIService;
	...
	public String consult(@RequestBody CalculoRebateMSI calculoRebateMSI
		...
		if (result.hasErrors()) {
			...
			return "fragments/rebatesMSI :: start";
		}
		...
		try {
			listCalculoRebates = this.iCalculoRebateMSIService.getCalculoRebateMSIView(calculoRebateMSI);
			if (listCalculoRebates.size() >= 1) {
				sesion.setCalculoRebateMSI(calculoRebateMSI);
				model.addAttribute("lista", listCalculoRebates);
				model.addAttribute("rebatesMSIFilter", calculoRebateMSI);
				return "fragments/rebatesMSI :: table";
			}
		} catch (ParseException e) {
			e.printStackTrace();
		}
		return "fragments/rebatesMSI :: noRecord";
	}
	...
	public void exportReportUser(HttpServletResponse response)
		...
		String name_report_user = nombreMes + anio.toString() + " RebatesMSI " + strDateComplete + ".xlsx";
		...
		try {
			List<CalculoRebateMSIDto> listCalculoRebates = this.iCalculoRebateMSIService.getCalculoRebateMSIView(calculoRebateMSI);
			ByteArrayInputStream stream = ExportCalculoRebateMSIExcel.calculoRebateMSIListToExcelFile(listCalculoRebates);
